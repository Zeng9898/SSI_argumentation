...
export default function StudentHomeSection({
  title, subtitle, count, collapsible = false, open, onToggle,
  className = '', accentColor = '#D08B2E', icon, children,
}: Props) {
  if (collapsible) {
    return (
      <section className={className}>
        <button type="button" onClick={onToggle} aria-expanded={open}
          ...
          <div className="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-full
                          bg-linear-to-b from-[#A8D88E] to-[#7DB044] border-2 border-[#5C8A2E]
          ...
          <div className={`flex-shrink-0 w-9 h-9 sm:w-10 sm:h-10 rounded-full
                          bg-linear-to-b from-[#F4D58A] to-[#F0B962] border-[3px] border-[#9B5E18]
                          flex items-center justify-center
                          shadow-[0_3px_0_#9B5E18] group-hover:shadow-[0_1px_0_#9B5E18]
                          transition-all duration-300
                          ${open ? 'rotate-180' : ''}`}>
            <Icon name="expand_more" filled className="text-xl sm:text-2xl text-[#7A4A18] drop-shadow-[0_1px_0_rgba(255,255,255,0.5)]" />
          </div>
        </button>
        {open && <div className="mt-3 sm:mt-4 animate-fade-up">{children}</div>}
      </section>
    );
  }
  ...
  return (
    <section className={className}>
      <div className="flex items-start gap-2 mb-3">
        <span className="block w-1 sm:w-1.5 self-stretch min-h-[1.5rem] mt-1 rounded-full" style={{ backgroundColor: accentColor }} />
        <div className="flex-1 leading-tight">
          <h2 className="font-bold text-base sm:text-lg text-[#5A3E22] flex items-center gap-2">
            {icon && <span className="material-symbols-rounded filled text-xl" style={{ color: accentColor }}>{icon}</span>}
            {title}
            {count != null && (
              <span className="inline-flex items-center justify-center min-w-[1.5rem] px-2 h-6 rounded-full
          ...
          {subtitle && <p className="text-xs sm:text-sm text-[#7A5232] mt-1">{subtitle}</p>}
        </div>
      </div>
      {children}
    </section>
