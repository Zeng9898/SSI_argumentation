    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b-[3px] border-[#C19A6B] flex-shrink-0
                        bg-linear-to-r from-[#C19A6B] to-[#8B5E3C]">
          <div className="flex items-center gap-2">
            <Icon name="settings" filled className="text-2xl text-[#FFF8E7]" />
            <h2 className="font-game text-xl font-bold text-[#FFF8E7] tracking-wide drop-shadow-[0_2px_0_rgba(91,66,38,0.5)]">
              設定
            </h2>
          </div>
          <button type="button" aria-label="關閉設定" onClick={onClose}
            className="w-9 h-9 rounded-full bg-white/20 hover:bg-white/30 text-white cursor-pointer
                ...
                return (
                  <button key={opt.value} type="button"
                    ...
                    className={`flex flex-col items-center gap-1 py-3 rounded-2xl border-[3px] cursor-pointer
                               ...
                               ${active
                                 ? 'border-[#D08B2E] bg-linear-to-b from-[#FFF1D8] to-[#F0B962]/30 scale-105 shadow-[0_3px_0_-1px_#9B5E18,0_4px_8px_-2px_rgba(155,94,24,0.3)]'
                                 : 'border-[#C19A6B]/50 bg-white/60 hover:border-[#C19A6B] hover:scale-[1.03]'
                               }`}>
                    <Icon name={FONT_SIZE_ICONS[opt.value]} filled
                      className={`text-2xl ${active ? 'text-[#D08B2E]' : 'text-[#8B5E3C]'}`} />
                    <span className={`text-sm ${active ? 'text-[#9B5E18]' : 'text-[#7A5232]'}`}>{opt.label}</span>
                  </button>
                );
              })}
            </div>
          ...
          <SettingsSection icon="info" title="關於系統">
            <button type="button" onClick={() => setAboutOpen((v) => !v)}
              className="w-full flex items-center justify-between py-2 px-1 text-left cursor-pointer
                         text-[#5A3E22] font-bold text-sm hover:text-[#D08B2E] transition-colors duration-200">
              <span>查看使用說明</span>
              <Icon name="expand_more" filled className={`text-xl text-[#7A5232] transition-transform duration-300 ${aboutOpen ? 'rotate-180' : ''}`} />
            </button>
            {aboutOpen && (
...
function SettingsSection({ icon, title, children }: { icon: string; title: string; children: React.ReactNode }) {
  const outer = WOOD_OUTER
    .replace('rounded-[28px]', 'rounded-[20px]')
    .replace('shadow-[0_6px_0_-1px_#5A3E22,0_14px_24px_-6px_rgba(91,66,38,0.45)]', 'shadow-[0_4px_0_-1px_#5A3E22,0_8px_16px_-4px_rgba(91,66,38,0.3)]');
  const inner = WOOD_INNER_CREAM.replace('rounded-[22px]', 'rounded-[15px]');
  return (
    <div className={outer}>
